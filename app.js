// app.js (ESM) — Charly Cripto • MVP
// -----------------------------------------------------------
// 1) DÓLAR: 6 widgets. CriptoYa via /proxy  (evita CORS)
// 2) EXCHANGES: Binance, Bybit, OKX funcionando
// 3) TradingView: cambia con botones de par
// -----------------------------------------------------------
...
const dollarState = {
  ...
  tarjeta: { compra: null, venta: null }, // solidario/ahorro
  mep:     { compra: null, venta: null },
  ccl:     { compra: null, venta: null },
  cripto:  { valor: 1600 } // editable por input
};

// Orden y nombres a mostrar
const DOLS = [
...
// Llamada a CriptoYa para llenar oficial/blue/mep/ccl/solidario
async function loadDollarsFromAPI() {
  try {
    const url = '/proxy?url=' + encodeURIComponent('https://criptoya.com/api/dolar');
    const r = await fetch(url);
    const d = await r.json();

    // Mapeos (manejo defensivo por si falta algún campo)
    dollarState.oficial = {
      compra: d?.oficial?.bid ?? null,
      venta:  d?.oficial?.ask ?? null
    };
    dollarState.blue = {
      compra: d?.blue?.bid ?? null,
      venta:  d?.blue?.ask ?? null
    };
    dollarState.mep = {
      compra: d?.mep?.bid ?? null,
      venta:  d?.mep?.ask ?? null
    };
    dollarState.ccl = {
      compra: d?.ccl?.bid ?? null,
      venta:  d?.ccl?.ask ?? null
    };
    // “Tarjeta” suele venir como “solidario”
    ...
    console.warn("No pude cargar dólar desde CriptoYa:", e.message);
    // Si falla, igual pintamos con lo que haya (quedarán guiones)
  } finally {
    paintDollarBoxes();
  }
}

// Input para setear USDT≈ARS manual
...
async function fetchOne(ex, pair) {
  if (ex.todo) {
    return { ex: ex.name, symbol: ex.sym ? ex.sym(pair) : pair, usd: null, ars: null, note: "configurar" };
  }
  try {
...
function mountTV(pair) {
  document.getElementById("tvchart").innerHTML = ""; // limpia
...
// ---------- Init ----------
paintDollarBoxes();      // pinta layout
loadDollarsFromAPI();   // trae CriptoYa y repinta
loadQuotes();           // cotizaciones de exchanges
mountTV(currentPair);   // gráfico

// Demo de análisis MA (placeholder)
document.getElementById("btnMA").onclick = () => {
  ...
  out.textContent = `Cuando conectemos datos del gráfico, calculo cruces de MA(${s}) y MA(${l}) para dar señal.`;
};
