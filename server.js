...
// Servir estáticos (Muro.jpeg en raíz)
app.use(express.static(__dirname));

// Index
app.get("/", (_req, res) => res.sendFile(path.join(__dirname, "index.html")));

// ——— DÓLAR (CriptoYa) con parseo tolerante ———
app.get("/api/dolar", async (_req, res) => {
  try {
    const r = await fetch("https://criptoya.com/api/dolar", { timeout: 10000 });
    const j = await r.json();

    // Algunos campos llegan como número, string o dentro de {v, ask, bid}.
    const pick = (x) => {
      if (x == null) return null;
      if (typeof x === "number") return x;
      if (typeof x === "string") return Number(x.replace(/[^\d.,-]/g, "").replace(".", "").replace(",", "."));
      if (typeof x === "object") {
        const cand = x.v ?? x.venta ?? x.sell ?? x.ask ?? x.promedio ?? x.blue ?? x.price ?? null;
        return pick(cand);
      }
      return null;
    };
    ...
    const out = {
      oficial: pick(j?.oficial),
      tarjeta: pick(j?.tarjeta ?? j?.solidario ?? j?.qatar),
      blue:   pick(j?.blue),
      mep:    pick(j?.mep),
      ccl:    pick(j?.ccl)
    };
...
app.get("/api/ticker", async (req, res) => {
  try {
    ...
    switch (exchange) {
      ...
      case "bitget": { // SPOT
        // Bitget spot usa símbolo simple en /api/spot/v1/market/ticker
        url = `https://api.bitget.com/api/spot/v1/market/ticker?symbol=${symbol}`;
        pick = async (r) => (await r.json())?.data?.close ?? (await r.json())?.data?.lastPr;
        break;
      }
      default:
    ...
    const rr = await fetch(url, { timeout: 10000 });
...
app.listen(PORT, () => console.log("Charly Cripto • MVP escuchando en :" + PORT));
